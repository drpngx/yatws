...
fn parse_company_info_from_issue(
  reader: &mut Reader<&[u8]>,
  end_tag_name: &[u8], // The name of the tag that signifies the end of this section (e.g., b"Issue")
  mut company_info: CompanyInformation,
) -> Result<CompanyInformation, IBKRError> {
  let mut buf = Vec::new();
  // Depth tracking to ensure we only parse CoIDs directly under this Issue's CoIDs tag
  let mut coids_depth = 0;
    ...
    match reader.read_event_into(&mut buf) {
      Ok(Event::Start(e)) => {
        match e.name().as_ref() {
          b"CoIDs" => {
            coids_depth += 1;
            // Only parse if this is the top-level CoIDs for this Issue
            if coids_depth == 1 {
              let mut coid_buf = Vec::new();
              loop {
                match reader.read_event_into(&mut coid_buf) {
                  Ok(Event::Start(coid_e)) if coid_e.name().as_ref() == b"CoID" => {
                    let coid_type_opt = get_attr_value(&coid_e, b"Type")?;
                    if let Some(coid_type) = coid_type_opt {
                      if let Ok(Event::Text(text_e)) = reader.read_event_into(&mut coid_buf) {
                        let value = text_e.unescape().map_err(|e| IBKRError::ParseError(e.to_string()))?.into_owned();
                        match coid_type.as_str() {
                          "CompanyName" => company_info.company_name = Some(value),
                          "CIKNo" => company_info.cik = Some(value),
                          "IRSNo" => company_info.irs_number = Some(value),
                          "CONID" => company_info.con_id = value.parse().ok(),
                          "SIC" => company_info.sic_code = Some(value),
                          _ => {}
                        }
                      }
                    }
                  }
                  Ok(Event::End(end_e)) if end_e.name().as_ref() == b"CoIDs" => {
                    coids_depth -= 1;
                    break;
                  }
                  Ok(Event::Eof) => return Err(IBKRError::ParseError("EOF in CoIDs".into())),
                  Err(err) => return Err(IBKRError::ParseError(format!("XML error in CoIDs: {}", err))),
                  _ => {}
                }
                coid_buf.clear();
              }
          ...
          b"IssueID" => {
            let id_type_opt = get_attr_value(&e, b"Type")?;
            if let Some(id_type) = id_type_opt {
              if let Ok(Event::Text(text_e)) = reader.read_event_into(&mut buf) {
                let value = text_e.unescape().map_err(|e| IBKRError::ParseError(e.to_string()))?.into_owned();
                if id_type == "Ticker" {
                  company_info.ticker = Some(value);
                }
              }
            }
          }
          b"Exchange" => {
            company_info.exchange_code = get_attr_value(&e, b"Code")?;
            if let Ok(Event::Text(text_e)) = reader.read_event_into(&mut buf) {
              company_info.exchange_name = Some(text_e.unescape().map_err(|e| IBKRError::ParseError(e.to_string()))?.into_owned());
            }
          }
          b"MostRecentSplit" => {
            company_info.last_split_date = parse_optional_date(get_attr_value(&e, b"Date")?);
            if let Ok(Event::Text(text_e)) = reader.read_event_into(&mut buf) {
              company_info.last_split_ratio = Some(text_e.unescape().map_err(|e| IBKRError::ParseError(e.to_string()))?.into_owned());
            }
          }
          _ => { /* Skip other tags or delve deeper */ }
        }
      }
      Ok(Event::End(e)) => {
        if e.name().as_ref() == b"CoIDs" && coids_depth > 0 {
          coids_depth -=1; // Decrement if we are exiting a nested CoIDs
        } else if e.name().as_ref() == end_tag_name { // Compare with the passed end_tag_name
          break;
        }
      }
      Ok(Event::Eof) => return Err(IBKRError::ParseError(format!("EOF in section supposed to end with </{}>", String::from_utf8_lossy(end_tag_name)))),
      Err(err) => return Err(IBKRError::ParseError(format!("XML error in Issue: {}", err))),
      _ => {}
    }
    buf.clear();
  }
