...
pub fn process_message(handler: &mut MessageHandler, data: &[u8]) -> Result<(), IBKRError> {
  ...
  match msg_type {
    ...
    10 => process_contract_data(&handler.data_ref, &mut parser)?,
    11 => process_execution_data(&handler.account, &mut parser)?,
    ...
    18 => process_bond_contract_data(&handler.data_ref, &mut parser)?,
    19 => process_scanner_parameters(&handler.data_market, &mut parser)?,
    20 => process_scanner_data(&handler.data_market, &mut parser)?,
    21 => process_tick_option_computation(&handler.data_ref, &mut parser)?,
    ...
    79 => process_symbol_samples(&handler.data_ref, &mut parser)?,
    80 => process_mkt_depth_exchanges(&handler.data_market, &mut parser)?,
